...
### 2026-08-13 (поздний вечер) — финальный чек-лист по ТЗ (28 проверок)
- **Найден и исправлен баг отложенной публикации.** Финальный прогон по ТЗ выявил: новость с будущим `publishAt` получала `status: 'draft'` и поэтому никогда не появлялась в публичном списке даже после наступления даты — отложенная публикация фактически не работала. Причина: старая семантика «отложить = draft» конфликтовала с публичным фильтром `status === 'published' && publishAt <= now`.
- **Решение (в `news.controller.js`):** создание/обновление с любой датой `publishAt` теперь ставит `status: 'published'`; до наступления даты новость скрыта публичным фильтром (`publishAt <= now`), наступление даты автоматически делает её видимой. Черновик без даты по-прежнему `status: 'draft'` и никому не показывается. Это чистая семантика ADR 0001 («публикация — данные, а не таймер»).
- **Синхронизирован бейдж в `frontend/src/components/news/NewsCard.tsx`:** «Отложено» теперь определяется первой проверкой `publishAt > now` (отложенные имеют status published, но скрыты до даты), затем «Опубликовано», затем «Черновик».
- **Финальный прогон по ТЗ (28 проверок):** регистрация/логин/JWT с userId, middleware (401 без/с битым токеном), CRUD, отложенная публикация (создание → скрыта → опубликована по дате), CORS, upload + статика, socket-клиент, стек зависимостей — **28/28 зелёный, exit 0**.
- Обновлены `PROGRESS.md` (9.8 = ✅) и этот `CHANGELOG.md`.

### 2026-08-13 (доработка UI) — кликабельные уведомления
- **Колокольчик:** строки уведомлений теперь кликабельны — по клику открывается страница новости (`/news/:id`). Для этого в `NotificationsProvider.tsx` в `NotificationItem` добавлено поле `newsId` (берётся из `data.id` socket-события), а в `Bell.tsx` каждая строка обёрнута в `<Link>` (для `created`/`updated`; `deleted` — обычная строка, т.к. страницы уже нет). Попап закрывается после клика. Стиль `.bell__link` (hover + focus-visible) — в `_components.scss`.
- **Проверка:** `npm run lint` → 0 errors; `npm run build` → зелёный.
- Обновлены `PROGRESS.md` (9.11 = ✅) и этот `CHANGELOG.md`.

### 2026-08-13 (расширение функционала) — пул поддерживаемых файлов
- Расширен MIME-фильтр в `backend/src/config/upload.js`: добавлены **видео** (`video/mp4`, `video/webm`, `video/quicktime` — .mp4/.webm/.mov) и **текст/Markdown** (`text/plain`, `text/markdown` — .txt/.md) поверх уже поддерживаемых изображений (jpeg/png/webp/gif) и документов (pdf/doc/docx).
- Лимит размера файла поднят: **5 МБ → 50 МБ** (видео весит больше картинок/документов).
- Проверка на живом сервере: markdown → 201 + url; mp4 (видео) → 201 + url; запрещённый тип .zip → 400. Статика раздаётся через ранее добавленный прокси `/uploads` в `next.config.ts`.
- Обновлены `PROGRESS.md` (9.10 = ✅) и этот `CHANGELOG.md`.

### 2026-08-13 (фикс ручного теста) — изображения 404 на странице новости
- **Симптом:** пользователь загрузил картинку, но на странице новости она не отображалась: `Failed to load resource: 404`. Файл физически был на backend (`backend/uploads/...`), `GET /uploads/...` на :5000 → 200.
- **Причина:** в блок сохранён относительный url `/uploads/<файл>` (от backend :5000). Браузер открывает фронтенд на :3000, и запрос `/uploads/...` уходил на Next.js — там такого маршрута нет → 404.
- **Решение (frontend/next.config.ts):** добавлен `rewrites()` — любой запрос `/uploads/:path*` Next.js прозрачно проксирует на backend (`NEXT_PUBLIC_API_URL/uploads/:path*`). Теперь браузер получает файл через :3000 как «свой» (проверка: `curl :3000/uploads/...` → 200, заголовок `x-powered-by: Express` — ответ пришёл от backend через прокси). Сборка `npm run build` зелёная.
- Обновлены `PROGRESS.md` (9.9 = ✅) и этот `CHANGELOG.md`.

### 2026-08-13 (fix) — таймзона отложенной публикации + отображение времени
- **Таймзона:** `Editor.tsx` раньше отправлял `publishAt` как локальную строку без таймзоны; сервер интерпретировал её по своей таймзоне (на проде в UTC это сдвигало момент). Теперь отправляется `new Date(publishAt).toISOString()` — браузер парсит строку `datetime-local` как **локальное время юзера** и переводит в абсолютный ISO (UTC). Live-тест: сервер сохранил ISO+Z, статья скрыта до наступления, появилась ровно в момент.
- **Отображение времени:** в карточках `NewsCard.tsx` и на странице `/news/[id]` теперь показывается дата **и время** публикации (включая предстоящую у отложенных) в локальной таймзоне юзера — вместо только даты.
- Обновлены `PROGRESS.md` (9.15–9.16 = ✅) и этот `CHANGELOG.md`.

### 2026-08-13 (финальный фикс) — «Please enter a URL» при загрузке файлов
- **Симптом:** после загрузки файла с ПК и нажатия «Сохранить черновик» / «Отложить публикацию» браузер показывал «Please enter a URL» и не отправлял форму.
- **Причина:** поля `url` в блоках `ImageBlock` и `FileBlock` были `<input type="url">`. Браузерная валидация требует абсолютный URL (схема http/https), а загрузка возвращает **относительный** путь `/uploads/<файл>` — форма не отправлялась.
- **Решение:** в `frontend/src/components/editor/blocks/ImageBlock.tsx` и `FileBlock.tsx` тип заменён на `type="text"` (относительный путь `/uploads/...` валиден — его проксирует `next.config.ts`). Загрузка файла → сохранение черновика / отложенная публикация теперь работают.
- **Проверка:** пользователь протестировал — всё собирается. Обновлены `PROGRESS.md` (9.14 = ✅) и этот `CHANGELOG.md`.

### 2026-08-13 (доработки по ручному тесту) — вкладка «Отложенные», фиксы черновика/файлов
- **Запоминание вкладки главной:** `frontend/src/app/page.tsx` — выбранная вкладка («Мои», «Отложенные» и т.д.) сохраняется в `localStorage` (`winbd_news_view`); при возврате со страницы новости пользователь остаётся в той же категории. Для гостей вкладки «Мои»/«Отложенные» недоступны.
- **Вкладка «Отложенные» (только авторизованным):** backend — `GET /api/news?scheduled=1` (требует JWT, фильтр `{ status: 'published', publishAt: { $gt: now } }`, все авторы); `getNewsById` — авторизованный видит чужую отложенную статью (GET 200), но PUT/DELETE по-прежнему 403 для не-автора. Frontend — третья вкладка, карточка показывает бейдж «Отложено» без кнопок управления.
- **Фикс загрузки файлов:** `UploadButton.tsx` — убран принудительный `Content-Type: multipart/form-data` без boundary; axios теперь проставляет корректный `boundary`, multer парсит файл. Черновик с загруженными файлами сохраняется.
- **Разделение «Черновик» и «Отложенная»:** `Editor.tsx` — три действия: «Сохранить черновик» (без publishAt → status draft), новая кнопка «Отложить публикацию» (требует выбранную дату), «Опубликовать сейчас». Черновик больше не может незаметно стать отложенным.
- **Проверки:** frontend `tsc`/`lint` (0 ошибок)/`build` зелёные; backend live: ?scheduled=1 содержит отложенную ✓, черновик в scheduled не попадает ✓, чужой GET 200 ✓, чужой PUT 403 ✓, scheduled без токена 401 ✓.
- Обновлены `PROGRESS.md` (9.10–9.13 = ✅) и этот `CHANGELOG.md`.
